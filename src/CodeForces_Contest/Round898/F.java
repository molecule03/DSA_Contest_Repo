...
public class F {
    ...
    public static void main(String[] args) {
        ...
        for (int tt=0; tt<T; tt++) {
            ...
            for (int i = 0; i < n - 1; i++) {
                if (h.get(i) % h.get(i + 1) != 0) {
                    list.add(new Pair<>(j, i));
                    j = i + 1;
                }
            }
            list.add(new Pair<>(j, n - 1));

            int ans = 0;
            for (Pair<Integer, Integer> val : list) {
                ans = Math.max(ans, sol(val, a, k));
            }
    ...
    public static int sol(Pair<Integer, Integer> p, ArrayList<Integer> a, int k) {
        int i = p.getKey();
        int j = i;
        int ans = 0, sum = 0;

        for (int n = p.getValue(); j <= n; j++) {
            sum += a.get(j);
