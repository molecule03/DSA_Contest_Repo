package CodeForces_Contest.Round1009;
...
public class D{
    ...
    public static void main(String[] args) {
        ...
        for (int tt=0; tt<T; tt++) {
            int n = fs.nextInt();
            int m = fs.nextInt();
            long[] centers = fs.longreadArray(n);
            int[] radii =  fs.intreadArray(n);

            Map<Long, Integer> map = new HashMap<>();
//            Map<Long, Long> map
            for (int i = 0; i < n; i++) {
                long curX = centers[i];
                long curRadi = radii[i];
                long left = curX - curRadi;
                long right = curX + curRadi;
                for (long j = left; j <= right; j++) {
                    long dirX = j - curX;
                    long dirY = curRadi * curRadi - dirX * dirX;
//                    out.println(dirX+" "+dirY);
                    int yRange = (int) Math.floor(Math.sqrt(dirY));
//                    out.println(yRange);
                    int maxValue = Math.max(map.getOrDefault(j, 0), yRange);
...
//                    out.println(maxValue);
            ...
            long res = 0;
            for (int value : map.values()) res += 2L * value + 1;
            out.println(res);
    ...
    static Set<Long> printDivisorsInSet(long n) {
        Set<Long> list = new HashSet<>();
        for (long i=1; i*i<=n; i++){ if (n%i==0){list.add(i); list.add(n/i);} } return list;
    }
