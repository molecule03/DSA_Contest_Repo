package CodeForces_Contest.Round896;
...
public class B {
    ...
    public static void main(String[] args) {
        ...
        for (int tt=0; tt<T; tt++) {
            ...
            Pair<Long, Long>[] cod = new Pair[(int) (n + 1)];
            cod[0] = new Pair<>(0L, 0L);
            for (int i = 0; i < n; i++) {
                cod[i + 1] = new Pair(fs.nextLong(), fs.nextLong());
            }
            if(a <= k && b <= k){
                out.println(0);
                continue;
            }
            long majCit1 = Long.MAX_VALUE;
            long majCit2 = Long.MAX_VALUE;
            for (int i = 1; i <= k; i++) {
                majCit1 = Math.min(majCit1, minD(cod[i], cod[a]));
                majCit2 = Math.min(majCit2, minD(cod[i], cod[ b]));
            }
            long res = minD(cod[a], cod[ b]);
            if (majCit1 == Long.MAX_VALUE) {
                out.println(res);
            }
            else {
                out.println(Math.min(majCit1 + majCit2, res));
            }
    ...
    public static long minD(Pair<Long, Long> p1, Pair<Long, Long> p2) {
