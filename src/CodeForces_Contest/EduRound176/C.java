package CodeForces_Contest.EduRound176;
...
public class C {
    ...
    public static void main(String[] args) {
        ...
        for (int tt=0; tt<T; tt++) {
            int n = fs.nextInt();
            int m = fs.nextInt();
            int arr[] = fs.intreadArray(m);
            Arrays.sort(arr);
            long ans = 0;
            for (int k = 1; k < n; k++) {
                int lVal = m - binSearch(arr, k);
                int rVal = m - binSearch(arr, n - k);
                long val = (long) (Math.max(lVal, rVal) - 1) * Math.min(lVal, rVal);
                ans += val;
            }
