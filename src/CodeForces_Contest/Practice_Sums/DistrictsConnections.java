package CodeForces_Contest.Practice_Sums;
...
public class DistrictsConnections {
        ...
        for (int tt=0; tt<T; tt++) {
            ...
            for(int curNode=0; curNode<n; curNode++){
                ...
                for(int adjNode=0; adjNode<n; adjNode++){
                    if(curNode == adjNode || vis.contains(adjNode)) continue;
                    if(arr[curNode] != arr[adjNode]){
                        vis.add(adjNode);
                        res.add(new int[]{(curNode+1), (adjNode+1)});
                    }
                }
            }
            if(res.size() == n-1){
