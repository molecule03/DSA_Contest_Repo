package CodeForces_Contest.PinelyRound2;
...
//    }
    ...
    public static int findMissingElement(int[] arr, int n) {
        ...
        int missingElement = xorTotal ^ xorArray;
        return missingElement;
    }

    static final int MAX_VALUE = 100001; // Maximum value of elements
    ...
    static class FenwickTree {
        ...
        int query(int index) {
            ...
            while (index > 0) {
                result += BIT[index];
                index -= index & -index; // Move to the previous segment
            }
            return result;
        }
    }

    public static int calculateMex(int[] collection) {
        int n = collection.length;
        FenwickTree bit = new FenwickTree(MAX_VALUE);
