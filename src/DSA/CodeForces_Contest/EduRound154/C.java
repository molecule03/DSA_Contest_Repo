package DSA.CodeForces_Contest.EduRound154;
...
//            String a = scanner.next();
    ...
    private static boolean isValidExpression(String s) {
        ...
        for (int i = 0; flag && i < n; i++) {
            char cur = s.charAt(i);
            if (cur == '+') list[l++] = 0;
            else if (cur == '-') { l--; if (r == l) r--; }
            else if (cur == '1') { for (int j = r + 1; j < l; j++) { if (list[j] == 2) { flag = false; break; } list[j] = 1; } r = l - 1; }
            else if (l <= 1 || list[l - 1] == 1) { flag = false; }
            else list[l - 1] = 2;
        }
