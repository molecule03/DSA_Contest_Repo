package CodeForces_Contest.EduRound153;
...
//            }
            ...
            List<String> validParentheses = generateValidParentheses(n, s);
//            out.println("All valid parentheses of length " + n + ":");

            boolean flag = false;
            if(validParentheses.isEmpty()){
                out.println("No");
            }
            else {
                out.println("Yes");
                out.println(validParentheses.get(0));
            }
    ...
    public static List<String> generateValidParentheses(int n, String forbiddenSubstring) {
        List<String> validParentheses = new ArrayList<>();
        backtrack(validParentheses, "", 0, 0, n, forbiddenSubstring);
        return validParentheses;
    }

    private static void backtrack(List<String> result, String current, int open, int close, int n, String forbidden) {
        ...
        if (open < n) {
            backtrack(result, current + '(', open + 1, close, n, forbidden);
        }
        if (close < open) {
            backtrack(result, current + ')', open, close + 1, n, forbidden);
        }
    }
